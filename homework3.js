        ...
        document.getElementById('resetButton').addEventListener('click', function() {
            ...
            document.getElementById('lambda').value = 0.7;
        ...
        document.getElementById('startButton').addEventListener('click', function() {
            ...
            const p = n > 0 ? lambda / n : 0; // Avoid division by zero

            // Input validation
            if (n < 0 || m < 0 || lambda < 0) {
                alert("Please enter valid values: n and m must be >= 0, λ must be >= 0.");
                return;
            }
        ...
        function runSimulation(n, m, p) { 
            ...
            const { mean, variance } = calculateStatistics(penCounts); // Calculate statistics
            plotFrequencyDistribution(penCounts, n, mean, variance); // Plot frequency distribution
        }
        ...
        function calculateStatistics(penCounts) {
            let mean = 0;
            let variance = 0;

            for (let i = 0; i < penCounts.length; i++) {
                const count = penCounts[i];
                const delta = count - mean;
                mean += delta / (i + 1);
                variance += delta * (count - mean);
            }

            // Variance needs to be divided by the number of counts for unbiased estimate
            variance /= penCounts.length;
        ...
        function plotFrequencyDistribution(penCounts, n, mean, variance) { // Function to plot frequency distribution
            ...
            frequencyChartInstance = new Chart(ctx, { // Create a new chart
                ...
                options: {
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: `Mean: ${mean.toFixed(2)}, Variance: ${variance.toFixed(2)}` // Show mean and variance
                        }
                    },
                    scales: {
